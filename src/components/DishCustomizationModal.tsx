...
export const DishCustomizationModal: React.FC<DishCustomizationModalProps> = ({
  ...
  }, [isOpen, item, initialKitchenNote, applicableGroups]);
  ...
  const finalUnitPrice = Math.max(0, item.price + totalPriceDelta);
  ...
  const quickNoteTemplates =
    lang === 'zh-TW'
      ? ['急件請快出', '先上餐點', '附外帶盒', '分拆裝盤', '不加任何辛香料', '醬汁多給一份']
      : lang === 'ja'
      ? ['急ぎ', '先出し', 'テイクアウト包装', '薬味なし', 'ソース別添え', 'ソース多め']
      : ['Rush Order', 'Serve First', 'Takeaway Box', 'Separate Plates', 'No Spices', 'Extra Sauce'];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 p-2 sm:p-4 backdrop-blur-xs">
      <div className="w-full max-w-2xl max-h-[90vh] my-auto flex flex-col rounded-3xl bg-white shadow-2xl dark:bg-gray-900 border border-gray-100 dark:border-gray-800 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="shrink-0 flex items-center justify-between border-b border-gray-100 dark:border-gray-800 p-3 sm:p-4 bg-gray-50/90 dark:bg-gray-800/60">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 rounded-2xl bg-orange-100 dark:bg-orange-950/60 text-[#FF8A00] border border-orange-200 dark:border-orange-900">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl text-gray-400 hover:text-gray-600 hover:bg-gray-200/60 dark:hover:bg-gray-700 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="shrink-0 bg-amber-50/70 dark:bg-amber-950/30 border-b border-amber-200/60 dark:border-amber-900/40 px-4 py-2 flex items-center justify-between text-xs">
          <div className="flex items-center space-x-2 text-amber-900 dark:text-amber-200 font-bold">
            <span>{texts.basePrice[lang]}: ${item.price.toFixed(2)}</span>
            {totalPriceDelta !== 0 && (
          ...
          <div className="flex items-center space-x-1.5">
            <span className="text-gray-500 dark:text-gray-400 font-bold">{texts.finalPrice[lang]}:</span>
        ...
        <div className="p-3 sm:p-4 overflow-y-auto space-y-4 flex-1 overscroll-contain touch-pan-y">
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        {isSingle ? texts.singleSelectNotice[lang] : texts.multiSelectNotice[lang]}
                      </span>
                    </div>
                    ...
                    {group.options.map(option => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-xl text-xs font-bold border transition-all text-left cursor-pointer ${
                            active
                              ? isSingle
                              ...
                              : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-200 dark:border-gray-700 hover:border-gray-400'
                          }`}
                        >
                          <div className="flex items-center space-x-2 truncate mr-1">
                            ...
                            <span className="truncate text-[11px] sm:text-xs">{optLabel}</span>
                          </div>

                          {/* Price Tag Indicator */}
                          {option.priceDelta !== undefined && option.priceDelta !== 0 && (
                            <span
                              className={`shrink-0 ml-1 px-1.5 py-0.5 rounded text-[10px] font-black ${
                                active
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-700 p-3.5 bg-white dark:bg-gray-800 space-y-2.5">
            <label className="block text-xs font-black text-gray-900 dark:text-white">
              {texts.kitchenNoteLabel[lang]}
            </label>

            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[10px] font-bold text-gray-400">{texts.quickNotes[lang]}</span>
            ...
            <input
              ...
              placeholder={texts.kitchenNotePlaceholder[lang]}
        ...
        <div className="shrink-0 sticky bottom-0 z-20 p-3 sm:p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/95 dark:bg-gray-800/95 backdrop-blur-xs flex items-center justify-between sm:justify-end space-x-3">
          ...
          >
            ...
            <span>
              {texts.confirmBtn[lang]} (${finalUnitPrice.toFixed(2)})
            </span>
          </button>
        </div>
